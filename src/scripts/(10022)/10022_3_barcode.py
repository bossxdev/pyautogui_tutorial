...
def run_test_case():
    ...
    try:
        ...
        write_text("099400147613603", delay=3, description="กรอกบาร์โค้ดส่วนแรก")
        state("automation_test", log_level="INFO", log_message="กรอกบาร์โค้ดส่วนแรกสำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("1101401205272", delay=20, description="กรอกบาร์โค้ดส่วนที่สอง")
        state("automation_test", log_level="INFO", log_message="รอกรหัส อ้างอิง1สำเร็จ")
        
        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("12567001171311267", delay=20, description="กรอกบาร์โค้ดส่วนที่สาม")
        state("automation_test", log_level="INFO", log_message="กรอกรหัส อ้างอิง2สำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("110.25", delay=6, description="กรอกบาร์โค้ดส่วนที่สาม")
