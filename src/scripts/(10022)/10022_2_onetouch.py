...
def run_test_case():
    ...
    try:
       ...
       # คลิกปุ่มผู้ว่าจ้าง
        ...
        log_state("S02_15test", log_level="INFO", log_message="กรอกรร.สาธิตฯปัญญาภิวัฒน์สำเร็จ")
        pyautogui.press('enter')

   # คลิกเลือกมูลนิธิเพื่อนหญิง
        click((298, 131), delay=5, description="คลิกเลือกรร.สาธิตฯปัญญาภิวัฒน์")
        log_state("S02_15test", log_level="INFO", log_message="คลิกเลือกรร.สาธิตฯปัญญาภิวัฒน์")
        time.sleep(10)
        click((361, 339), delay=5, description="คลิกเลือกค่าธรรมเนียมนักเรียนใหม่")
        log_state("S02_15test", log_level="INFO", log_message="คลิกเลือกค่าธรรมเนียมนักเรียนใหม่สำเร็จ")
        time.sleep(10)
        
        
        write_text("111111111119", delay=4, description="กรอกรหัส อ้างอิง1")
        log_state("automation_test", log_level="INFO", log_message="กรอกรหัส อ้างอิง1สำเร็จ")
        pyautogui.press('enter')

        write_text("12567001171311267", delay=4, description="กรอกรหัส อ้างอิง2")
        log_state("automation_test", log_level="INFO", log_message="กรอกรหัส อ้างอิง2สำเร็จ")
        pyautogui.press('enter')
