...
def run_test_case():
    ...
    try:
        ...
        write_text("099400007263501", delay=3, description="กรอกบาร์โค้ดส่วนแรก")
        state("automation_test", log_level="INFO", log_message="กรอกบาร์โค้ดส่วนแรกสำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("0138980105", delay=20, description="กรอกบาร์โค้ดส่วนที่สอง")
        state("automation_test", log_level="INFO", log_message="กรอกเลขที่ผู้ใช้นํ้าสำเร็จ")
        
        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("29116740", delay=20, description="กรอกบาร์โค้ดส่วนที่สาม")
        state("automation_test", log_level="INFO", log_message="กรอกรหัส อ้างอิง2สำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("200.75", delay=6, description="กรอกบาร์โค้ดส่วนที่สาม")
