...
def monitor_image_display(image_path, timeout=2):
    ...
    log_state("automation_test", log_level="WARNING", log_message="ไม่พบภาพที่ต้องการภายในเวลาที่กำหนด")
    return False
...
def run_test_case():
    ...
    try:
        ...
        log_state("automation_test", log_level="INFO", log_message="คลิกเลือกประกันอีซี่สำเร็จ")
        time.sleep(10)
        
        click((414, 553), delay=5, description="คลิกเลือกคีย์เลขที่บัตร")
        log_state("automation_test", log_level="INFO", log_message="คลิกเลือกคีย์เลขที่บัตรสำเร็จ")

        click((404, 539), delay=5, description="คลิกเลือกกรอกบัตรประชาชน")
        log_state("automation_test", log_level="INFO", log_message="คลิกเลือกกรอกบัตรประชาชนสำเร็จ")
        time.sleep(20)
        write_text("0997317465", delay=3, description="กรอกหมายเลขโทรศัพท์")
        log_state("automation_test", log_level="INFO", log_message="กรอกหมายเลขโทรศัพท์")
        pyautogui.press('enter')
