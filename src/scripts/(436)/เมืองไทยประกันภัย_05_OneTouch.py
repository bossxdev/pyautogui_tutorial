...
def run_test_case():
    try:
        ...
        click((80, 330), delay=3, description="กดปุ่มซื้อ/ชำระเบี้ยประกัน/พรบ")
        log_handler("test_case", "INFO", "กดปุ่มซื้อ/ชำระเบี้ยประกัน/พรบ")
        
        click((80, 220), delay=3, description="กดปุ่มชำระเบี้ยประกัน")
        log_handler("test_case", "INFO", "กดปุ่มชำระเบี้ยประกัน")
        
        click((700, 560), delay=5, description="กดปุ่มเมืองไทยประกันภัย")
        log_handler("test_case", "INFO", "กดปุ่มเมืองไทยประกันภัย")
        
        click((400, 280), delay=10, description="คลิกเลือก service")
        log_handler("test_case", "INFO", "คลิกเลือก service")
        
        click((400, 560), delay=50, description="คลิกคีย์เลขที่บัตร")
        log_handler("test_case", "INFO", "คลิกคีย์เลขที่บัตร")
        
        log_handler("test_case", "INFO", "กรอกรหัสบัตรประชาชนที่เครื่อง EDC")
