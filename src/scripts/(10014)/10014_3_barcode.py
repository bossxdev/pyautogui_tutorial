...
def run_test_case():
    ...
    try:
        ...
        write_text("010753700137403", delay=3, description="กรอกบาร์โค้ดส่วนแรก")
        state("automation_test", log_level="INFO", log_message="กรอกบาร์โค้ดส่วนแรกสำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("123456789", delay=20, description="กรอกบาร์โค้ดส่วนที่สอง")
        state("automation_test", log_level="INFO", log_message="กรอกรหัส Cashsaleสำเร็จ")
        
        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("1234567891", delay=20, description="กรอกบาร์โค้ดส่วนที่สาม")
        state("automation_test", log_level="INFO", log_message="กรอกรหัสพนักงานสำเร็จ")

        pyautogui.keyDown('shift')
        pyautogui.press('\\')
        pyautogui.keyUp('shift')
        write_text("50000", delay=6, description="กรอกบาร์โค้ดส่วนที่สาม")
