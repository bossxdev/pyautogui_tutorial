...
def run_test_case():
    ...
    try:
       ...
       # คลิกปุ่มผู้ว่าจ้าง
        ...
        log_state("S02_15test", log_level="INFO", log_message="กรอกบริษัท ไอ.ซี.ซี.อินเตอร์เนชั่นแนลจำกัดสำเร็จ")
        pyautogui.press('enter')

   # คลิกเลือกมูลนิธิเพื่อนหญิง
        click((298, 131), delay=5, description="คลิกเลือกบริษัท ไอ.ซี.ซี.อินเตอร์เนชั่นแนลจำกัด")
        log_state("S02_15test", log_level="INFO", log_message="คลิกเลือกบริษัท ไอ.ซี.ซี.อินเตอร์เนชั่นแนลจำกัดสำเร็จ")
        time.sleep(10)
        click((361, 339), delay=5, description="คลิกเลือกค่าสินค้า Cashsale")
        log_state("S02_15test", log_level="INFO", log_message="คลิกเลือกค่าสินค้า Cashsale สำเร็จ")
        time.sleep(10)
        
        write_text("123456789", delay=4, description="กรอกรหัส Cashsale")
        pyautogui.press('enter')

        log_state("automation_test", log_level="INFO", log_message="กรอกรหัส Cashsale สำเร็จ")
        
        write_text("1234567891", delay=4, description="กรอกรหัสพนักงาน")
        log_state("automation_test", log_level="INFO", log_message="กรอกรหัสพนักงานสำเร็จ")
