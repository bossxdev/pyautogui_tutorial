...
def monitor_image_display(image_path):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
         ...
         if pyautogui.locateOnScreen(image_path, confidence=0.6):  # เพิ่มค่าความมั่นใจหรือลดให้เหมาะสม
            state("automation_test", log_level="INFO", log_message="พบภาพที่คาดหวังแล้ว")
 
            return True  # พบภาพที่ต้องการแล้ว
        except Exception as e:
            print(f"เกิดข้อผิดพลาดในการจับภาพหน้าจอ: {e}")
            state("automation_test", log_level="ERROR", log_message=f"เกิดข้อผิดพลาดในการตรวจสอบภาพ: {e}")
    ...
    state("automation_test", log_level="ERROR", log_message="ไม่พบภาพที่คาดหวังภายใน 20 วินาที")
    return False
...
def run_test_case():
    ...
    try:
        ...
        if not monitor_image_display("EDC.png"):  # ตรวจสอบภาพหลังจากกรอกบาร์โค้ด
            state("automation_test", log_level="ERROR", log_message="ข้อมูลไม่ถูกต้อง")
            return
        # กดยืนยันบาร์โค้ด
        ...
        state("S02_15test", log_level="INFO", log_message="คลิกเลือกค่าสินค้าร้านค้า")
        pyautogui.press('enter')
        ...
        state("automation_test", log_level="INFO", log_message="กรอกเลขผู้เสียภาษาษีสำเร็จ")
        ...
        state("automation_test", log_level="ERROR", log_message=error_message)
        
        # แสดง stack trace เพื่อหาสาเหตุของข้อผิดพลาด
        traceback_message = traceback.format_exc()
        state("automation_test", log_level="ERROR", log_message=f"Stack Trace: {traceback_message}")
