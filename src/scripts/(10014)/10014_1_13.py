...
def monitor_image_display(image_path):
    ...
    state("automation_test", log_level="ERROR", message="ไม่พบภาพที่คาดหวังภายใน 20 วินาที")
    return False
...
def run_test_case():
    ...
    try:
        ...
        state("S02_15test", log_level="INFO", message="คลิกเลือกค่าสินค้าร้านค้า")
        pyautogui.press('enter')
        ...
        state("automation_test", log_level="ERROR", message=error_message)
        
        # แสดง stack trace เพื่อหาสาเหตุของข้อผิดพลาด
        traceback_message = traceback.format_exc()
        state("automation_test", log_level="ERROR", message=f"Stack Trace: {traceback_message}")
