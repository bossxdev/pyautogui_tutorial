...
def run_test_case():
    try:
        ...
        write_text("099400077225406", delay=2, description="กรอกบาร์โค้ด")
        log_state("test_case", "INFO", "กรอกบาร์โค้ด: 099400077225406")
        
        click((480, 464), delay=20, description="คลิกปุ่มยืนยัน pop up บาร์โค้ด")
        log_state("test_case", "INFO", "ยืนยัน pop up บาร์โค้ด")
        
        write_text("1749700015973", delay=3, description="")
        log_state("test_case", "INFO", "")

        pyautogui.press("enter")
        time.sleep(2)
        
        write_text("357200223115126760", delay=3, description="")
        log_state("test_case", "INFO", "")
        
        pyautogui.press("enter")
        time.sleep(2)
        
        write_text("90000", delay=3, description="กรอกจำนวนเงิน")
        log_state("test_case", "INFO", "กรอกจำนวนเงิน")
        ...
        log_state("test_case", "DEBUG", "การทดสอบเสร็จสมบูรณ์")
        
    except Exception as e:
        log_state("test_case", "DEBUG", f"เกิดข้อผิดพลาดระหว่างการทดสอบ: {str(e)}")
        raise
