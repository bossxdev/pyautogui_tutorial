...
def run_test_case():
    ...
    try:
       ...
       # คลิกปุ่มผู้ว่าจ้าง
        ...
        state("S02_15test", log_level="INFO", log_message="กรอกสำนักงานสัตวแพทยสภาสำเร็จ")
        pyautogui.press('enter')

   # คลิกเลือกมูลนิธิเพื่อนหญิง
        click((298, 131), delay=5, description="คลิกเลือกสำนักงานสัตวแพทยสภา")
        state("S02_15test", log_level="INFO", log_message="คลิกเลือกสำนักงานสัตวแพทยสภา")
        time.sleep(10)
        # click((361, 339), delay=5, description="คลิกเลือกค่าธรรมเนียมต่างๆ")
        # state("S02_15test", log_level="INFO", log_message="คลิกเลือกค่าธรรมเนียมต่างสำเร็จ")
        # time.sleep(10)
        
        
        write_text("A12345678", delay=4, description="กรอกรหัสกิจกรรม")
        state("automation_test", log_level="INFO", log_message="กรอกรหัสกิจกรรม")
        pyautogui.press('enter')

        write_text("1234", delay=4, description="กรอกเลขที่อ้างอิง1")
        state("automation_test", log_level="INFO", log_message="กรอกเลขที่อ้างอิง1")
        pyautogui.press('enter')
