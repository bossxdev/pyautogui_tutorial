...
def run_test_case():
    ...
    try:
        ...
        # state("S02_15test", log_level="INFO", message="คลิกเลือกค่าธรรมเนียมต่างสำเร็จ")
        # time.sleep(10)
        
        write_text("A12345678", delay=4, description="กรอกรหัสกิจกรรม")
        state("automation_test", log_level="INFO", message="กรอกรหัสกิจกรรม")
        pyautogui.press('enter')

        write_text("1234", delay=4, description="กรอกเลขที่อ้างอิง1")
        state("automation_test", log_level="INFO", message="กรอกเลขที่อ้างอิง1")
        pyautogui.press('enter')
